...
def run_dem_rsmas(argv):
    ...
    if os.path.isdir('DEM'):
       shutil.rmtree('DEM')
        ...
        if 'sentinelStack.demDir' in list(custom_template.keys()) and custom_template['sentinelStack.demDir'] != str('auto'):
            shutil.copytree(custom_template['sentinelStack.demDir'], 'DEM')
        else:
